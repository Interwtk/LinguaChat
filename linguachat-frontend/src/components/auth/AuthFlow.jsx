    ...
    <div style={{ minHeight: '100dvh', background: 'var(--bg)', display: 'flex', flexDirection: 'column' }}>
      ...
      {back && (
        <button onClick={onBack} className="flex items-center gap-1.5 mx-5 mt-4 self-start"
          style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--muted)', fontWeight: 600, fontSize: '0.875rem' }}>
...
function PasswordStrength({ password }) {
  ...
  const colors = ['', 'var(--accent)', 'var(--accent-tint)', 'var(--info)', 'var(--positive)']
  return (
    <div className="flex items-center gap-2 mt-1">
      ...
      <span style={{ fontSize: 11, fontWeight: 600, color: colors[score] || 'var(--muted)', minWidth: 36 }}>{labels[score]}</span>
    </div>
  )
}
  ...
  return (
    <AuthShell>
      <div className="flex flex-col items-center text-center animate-fade-up">
        ...
        <p style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.14em', color: 'var(--accent)', marginBottom: 14 }}>
          {t('entryEyebrow')}
        </p>
        ...
        <p style={{ fontSize: 11, color: 'var(--muted)', marginTop: 28, lineHeight: 1.6 }}>{t('entryNote')}</p>
      </div>
    </AuthShell>
  )
}
    ...
    <AuthShell back onBack={() => setAuthStep('entry')}>
      <div className="animate-fade-up">
          ...
          <div>
            ...
            <div className="flex justify-end mt-1.5">
              <button type="button" onClick={() => setAuthStep('forgot')}
                style={{ fontSize: '0.8125rem', color: 'var(--accent)', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 600 }}>
                {t('forgotPassword')}
              </button>
            </div>
          </div>
        ...
        <p style={{ textAlign: 'center', marginTop: 24, fontSize: '0.875rem', color: 'var(--muted)' }}>
          {t('newHere')}{' '}
          <button onClick={() => setAuthStep('signup')} style={{ color: 'var(--accent)', fontWeight: 700, background: 'none', border: 'none', cursor: 'pointer' }}>{t('startJourney')}</button>
        </p>
      </div>
    </AuthShell>
  )
  ...
  return (
    <AuthShell back onBack={() => setAuthStep('entry')}>
      <div className="animate-fade-up">
        ...
        <p style={{ textAlign: 'center', marginTop: 24, fontSize: '0.875rem', color: 'var(--muted)' }}>
          {t('alreadyPracticing')}{' '}
          <button onClick={() => setAuthStep('login')} style={{ color: 'var(--accent)', fontWeight: 700, background: 'none', border: 'none', cursor: 'pointer' }}>{t('signIn')}</button>
        </p>
      </div>
    </AuthShell>
  )
      ...
      <div className="animate-fade-up">
        {!sent ? (
          ...
          <div className="flex flex-col items-center text-center py-8">
            ...
            <p style={{ fontSize: '0.9375rem', color: 'var(--muted)', lineHeight: 1.6, marginBottom: 28 }}>{t('recoverySentText')}</p>
            <button onClick={() => setAuthStep('login')}
