...
import {
  PRE_A1, A1, A2, B1, LEVEL_IDS, playableLevelId, labelKeyOfLevel, isLevelAvailable,
...
async function main() {
  ...
  // 7) No B2+ curriculum was invented by this task: the registry knows
  //    exactly Pre-A1, A1, A2 and B1, in that order (A2/B1 wired in by
  //    LC-INT-001, per its own task description — not an invention this
  //    check should still be guarding against), and none of A1/A2/B1 is open.
  {
    assert.deepEqual(LEVEL_IDS, [PRE_A1, A1, A2, B1])
